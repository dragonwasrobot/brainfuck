        ...
        , Html.div [ Attr.class "mt-4 text-sm hyphens-auto" ]
            ...
            , Html.ul [ Attr.class "mt-2 ml-4 list-disc" ]
                [ Html.li [ Attr.class "mt-2" ]
                    ...
                    , Html.span [ Attr.class "ml-2" ] [ Html.text "Eight single-character instructions provide arithmetic on the working cell, bidirectional pointer motion, byte-stream input/output compatible with Model 33 ASR devices, and structured iteration via matched delimiters." ]
                    ]
                , Html.li [ Attr.class "mt-2" ]
                    [ Html.span [ Attr.class "font-semibold" ] [ Html.text "High transportability." ]
                    , Html.span [ Attr.class "ml-2" ] [ Html.text "Cell width, tape extent, and end-of-file conventions are all configurable at the time of program execution, permitting straightforward accommodation to diverse cores, drums, or disc subsystems while retaining program form — provided that the operator has correctly declared each of the foregoing parameters prior to translation." ]
                    ]
                , Html.li [ Attr.class "mt-2" ]
                ...
                , Html.li [ Attr.class "mt-2" ]
                    ...
                    , Html.span [ Attr.class "ml-2" ] [ Html.text "No symbol tables, stacks, or hidden temporaries are required beyond the working store and the input/output streams, rendering BF-4000 suitable for dynamic programming, numerical computation, and environments of constrained memory. Convenience features such as named variables, subroutines, and informative error messages have been omitted in the interest of clarity." ]
                    ]
                ]
            , Html.br [] []
            , Html.span [] [ Html.text "The following concise specification enumerates the syntax and semantics of the BF-4000 machine language in full. No companion volume is anticipated." ]
            ]
...
viewLanguageSpec =
    Html.div [ Attr.id "language-spec" ]
        [ Html.h2 [ Attr.class "font-semibold" ] [ Html.text "2.1.1 LANGUAGE SPECIFICATION" ]
...
viewInstructionSemanticsDescription =
    Html.div [ Attr.class "mt-4" ]
        ...
        , Html.div [ Attr.class "mt-2 text-sm hyphens-auto" ]
            [ Html.span [] [ Html.text "Table 3.1 below lists the symbols and semantics of the eight instructions of the BF-4000 machine language." ] ]
        ...
        , Html.div [ Attr.class "mx-4 mt-1 text-center text-sm" ]
            [ Html.span [ Attr.class "font-semibold" ] [ Html.text "Table 3.1" ] ]
        ]
...
viewAbstractMachineDescription =
    Html.div [ Attr.class "mt-4" ]
        ...
        , Html.ul [ Attr.class "mt-2 ml-4 list-disc text-sm hyphens-auto" ]
            [ Html.li [ Attr.class "mt-1" ] [ Html.text "The tape comprises a linear array of 30,000 cells, each of byte width." ]
            , Html.li [ Attr.class "mt-1" ] [ Html.text "A data pointer (DP) designates the active cell." ]
            , Html.li [ Attr.class "mt-1" ] [ Html.text "Two unidirectional byte streams furnish input and output, respectively." ]
            , Html.li [ Attr.class "mt-1" ] [ Html.text "Reference to a cell outside the tape extent is diagnosed (error E-07, POINTER ELSEWHERE); execution is then suspended." ]
            , Html.li [ Attr.class "mt-1" ] [ Html.text "Arithmetic overflow of a cell beyond its representable range is likewise diagnosed (error E-12, CELL UNREASONABLE)." ]
            , Html.li [ Attr.class "mt-1" ] [ Html.text "End-of-file on input leaves the active cell unaltered (this is not an error and consequently has no code)." ]
            ]
        ]
